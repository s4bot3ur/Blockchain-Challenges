...
contract Chal {
    ...
    function fight() public returns (string memory) {
        ...
        uint16 playerAttack = playerCharacter.attack + playerItem.attackBonus;//43+658=701
        uint16 playerDefense = playerCharacter.defense + playerItem.defenseBonus;//33+329=362

        uint16 bossAttack = boss.attack + boss.item.attackBonus;//250+89=339
        uint16 bossDefense = boss.defense + boss.item.defenseBonus;//250+58=308
    ...
    function randomStat(uint16 max, uint256 seed) public view returns (uint16) {
        return toUint16(
            uint256(keccak256(abi.encodePacked(block.timestamp, blockhash(block.number - 1), msg.sender, seed)))
        ) % max + 1;
    }

    function toUint8(uint256 n) public pure returns (uint8 result) {
        assembly {
            result := n
        }
    }

    function toUint16(uint256 n) public pure returns (uint16 result) {
        assembly {
            result := n
        }
    }
